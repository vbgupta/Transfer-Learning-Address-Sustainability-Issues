    ...
    if city == 'Chicago':
        ...
        if multi_week:
            ...
            for i in week.values():
                result_val = pd.DataFrame(data[(data["week"] == int([i])) & (data["year"] == int(year))])
                result_val = result_val.iloc[:, 1].values
                actual_val = pd.DataFrame(data[(data["week"] == int([i])) & (data["year"] == int(year))])
...
def app():
    ...
    if funct == 'Multiple weeks for given year':
        ...
        entered_weeks = st.button('Enter')
        if entered_weeks:
            count = 1
            for i in range(0, int(num_weeks)):
                week_dict[i] = st.number_input("Please enter the week of the year you would like to predict:",
                                               min_value=1, max_value=53, key=count)
                count += 1
